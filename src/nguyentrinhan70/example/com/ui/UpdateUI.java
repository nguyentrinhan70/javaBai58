...
public class UpdateUI extends JFrame {
	...
	protected void xuLyLuu() {
		// TODO Auto-generated method stub
		try{
			String sql ="update taisan set tenTs =?,NgayNhap=?,KhauHao = ?,GiaTri = ? where ma=? ";
			PreparedStatement preparedStatement = conn.prepareStatement(sql);
			
			preparedStatement.setString(1, txtTen.getText());
			preparedStatement.setString(2, txtNgayNhap.getText());
			preparedStatement.setInt(3, Integer.parseInt(txtKhauHao.getText()));
			preparedStatement.setInt(4, Integer.parseInt(txtGiaTi.getText()));
			preparedStatement.setString(5, txtMa.getText());
			int x = preparedStatement.executeUpdate();
			if(x>0){
				JOptionPane.showConfirmDialog(null, "Sửa tài sản mới thành công");
